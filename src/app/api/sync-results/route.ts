...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [{ data: matches }, { data: pools }] = await Promise.all([
      supabase.from('matches').select('id, home, away'),
    ...
    for (const apiMatch of data.matches ?? []) {
      ...
      // Preserve the official home/away order from our own matches table,
      // not the API's order (some venues report differently)
      const isFlipped = match.home === away

      // Write this result to every pool
      ...
      const { error } = await supabase
        .from('results')
        .upsert(rows, { onConflict: 'pool_id,match_id' })
